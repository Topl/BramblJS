...
Brambl.prototype.addSigToTx = async function (
  prototypeTx: PrototypeTx,
  userKeys: KeyManager
) {
  // function for generating a signature in the correct format
  const genSig = (keys: Array<any>, txBytes: any) => {
    return Object.fromEntries(
      keys.map((key: Key) => [key.pk, base58.encode(key.sign(txBytes))])
    );
  };
...
Brambl.prototype.transaction = async function (method: string, params: any) {
  ...
  return this.requests[method](params).then((res: { result: any }) =>

...
import { PrototypeTx, Options } from './Brambl.d';
...
class Brambl {
    ...
    constructor(params: any) {
        ...
        if (keyManagerVar.instance) {
            ...
            this.keyManager = new KeyManager(keyManagerVar);
        }
     ...
     * @memberof Brambl
     */
    static Requests(url: string, apiKey: string): Requests {
     ...
     * @memberof Brambl
     */
    static KeyManager(params: ConstructorParams): KeyManager {
     ...
     */
    addSigToTx = async function (prototypeTx: PrototypeTx, userKeys: KeyManager): Promise<any> {
        // function for generating a signature in the correct format
        const genSig = (keys: Array<KeyManager>, txBytes: string): { [key: string]: string } => {
            return Object.fromEntries(keys.map((key: KeyManager) => [key.pk, base58.encode(key.sign(txBytes))]));
        };
     ...
     * @param {object} prototypeTx An unsigned transaction JSON object
     */
    signAndBroadcast = async (prototypeTx: PrototypeTx): Promise<{ [key: string]: string }> => {
     ...
     */
    transaction = async (method: any, params: any): Promise<{ [key: string]: string }> => {
     ...
     */
    pollTx = async (txId: string, options: Options): Promise<{ [key: string]: string }> => {
